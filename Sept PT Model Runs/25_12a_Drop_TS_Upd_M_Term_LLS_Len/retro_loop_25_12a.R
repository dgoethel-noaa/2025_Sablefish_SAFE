...
mod_name_main <- '25_12a_Drop_TS_Upd_M_Term_LLS_Len'                                                    # Model name to use for saving files and labels
root_folder <- here("Sept PT Model Runs", mod_name_main)                                           # The root folder where runs will be stored (sub folders named with mod_name)
mod <- readRDS(here(root_folder,paste0(mod_name_main,"_input_list.RDS")))

comp_name <- "Incl_Term_LLS_Len"
...
mod_comp_name <- "Drop_TS_Upd_M"                                                  # name of model runs to read in
...
data_len <- readRDS(here(root_folder,"SPoRC_one_reg_age_joint.rds"))
...
for(i in 0:peels){
  ...
  if(i>0){                                                                     # fit the survey length comps in the terminal year
    data$UseSrvLenComps[,65-i,1] <- 1
    data$ISS_SrvLenComps[,65-i,,1] <- data_len$ISS_SrvLenComps[,65-i,,1]
    data$ObsSrvLenComps[,65-i,,,1] <- data_len$ObsSrvLenComps[,65-i,,,1]
  }
