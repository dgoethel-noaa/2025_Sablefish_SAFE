# Purpose: Spatial Sablefish Sensitivity Run - Multinomial Release Conditioned, Francis Reweighting, Tighter Survey Selex Priors
...
mod_name <- "Spatial_MltRel_Francis_TgtSelPr"
...
srv_selex_prior <- cbind(
  region = 1,
  srv_selex_structure,
  mu = 2, # shift priors by 1 and make sd smaller
  sd = 3 # shift priors by 1 and make sd smaller
) %>%
...
for(j in 1:10) {
  ...
  # get francis weights
  wts <- SPoRC::do_francis_reweighting(data = data, rep = rep, age_labels = input_list$data$ages,
  ...
  print(get_selex_plot(list(sabie_rtmb_model$rep), 1)[[1]])
  print(get_selex_plot(list(sabie_rtmb_model$rep), 1)[[2]])
