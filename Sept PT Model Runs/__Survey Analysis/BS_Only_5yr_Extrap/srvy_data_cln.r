...
load(here("Sept PT Model Runs","_Survey Analysis","Sablefish_Data_2024.RData"))
SPoRC_dat <- readRDS(here("Sept PT Model Runs","_Survey Analysis","SPoRC_one_reg_age_joint_no_len_kp_JPN.rds"))


 model_run <- "5yr Extrap"

new_years <- c(2015,2017,2019,2021,2023)
styr <- 1960
filt_reg <- "Bering Sea"
filt_reg_abb <- 'BS'
filt_reg_abb_alt <- 'AI'
...
lls_age_filter_sex_SPoRC_yrs <- lls_age_filter_sex_SPoRC_temp %>%
  dplyr::filter(c(year %in% c(new_years) & !council_sablefish_management_area == filt_reg))          # filter out BS in
...
lls_age_prop_sex_SPoRC <- left_join(lls_age_filter_sex_SPoRC, lls_area_rpn_filter) %>%   # joining age frequencies with rpns
  ...
  vroom::vroom_write(here::here("Sept PT Model Runs","_Survey Analysis",model_run, "lls_rpn_age_comp_prop_SPoRC.csv"),delim = ",")

for(i in new_years){
  lls_rpn_dep_area_temp_yrs_repl <- lls_rpn_dep_area_temp %>%
    dplyr::filter(year %in% (i-1) & area == filt_reg_abb_alt) %>%
    full_join(lls_rpn_dep_area_temp %>%
                dplyr::filter(year %in% (i-2) & area == filt_reg_abb)) %>%
    dplyr::mutate(year = i) %>%
    full_join(lls_rpn_dep_area_temp %>%
                dplyr::filter(year %in% i & !area == filt_reg_abb)) %>%
...
file_copy(here("Sept PT Model Runs","_Survey Analysis","srvy_data_cln.r"), here("Sept PT Model Runs","_Survey Analysis",model_run,"srvy_data_cln.r"), overwrite = TRUE)

saveRDS(SPoRC_dat, file = here("Sept PT Model Runs","_Survey Analysis",model_run,"SPoRC_one_reg_age_joint_no_len_kp_JPN.rds"))
