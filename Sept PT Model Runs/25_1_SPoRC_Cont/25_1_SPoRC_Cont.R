...
mod_name <- '25_1_SPoRC_Cont'                                              # Model name to use for saving files and labels
...
SPoRC_one_reg <- readRDS(here(root_folder,mod_name,"SPoRC_one_reg.rds"))
...
# Input data weights for catch, indices, penalty terms (these are fixed, so don't need to be updated each eyar)
Wt_Catch = 50
Wt_FishIdx = 0.448
Wt_SrvIdx = 0.448
Wt_Rec = 1.5
...
input_list <- Setup_Mod_Rec(input_list = input_list,                           # input data list from above
                            ...
                            ln_sigmaR = log(c(0.4, 1.2)),                      # early and late sigma_R starting values
                            rec_model = "mean_rec",                            # recruitment model, use mean recruitment (not BH SRR)
                            sigmaR_spec = "fix_early_est_late",                # fix early sigmaR, estimate late sigmaR
...
input_list <- Setup_Mod_Biologicals(input_list = input_list,
                                    ...
                                    M_offset = -0.00819813327864               # starting value for male M offset; fixing Male M offset (accidentally jittered from original assessment)
)
...
input_list <- Setup_Mod_Catch_and_F(input_list = input_list,
                                    ...
                                    Catch_Constant = c(0.01, 0.8)              # The small constant to add to the catch term to avoid log(0)--Fournier robust likelihood approach; fixed gear then trawl fishery
)
...
input_list <- Setup_Mod_FishIdx_and_Comps(input_list = input_list,
                                          ...
                                          FishAgeComps_Type =                  # age comp structure for fishery fleet 1 and 2 (agg_Year_1-terminal_Fleet_1 indicates that age comps are aggregated across sex for all years for fleet 1--fixed gear)
                                            c("agg_Year_1-terminal_Fleet_1",   # NOTE: these entries are needed for all years and all fleets, so if want to agg for some years then disagg for remaining years, would need two entries for that fleet
                                          ...
                                          FishAge_comp_agg_type = c(0,NA),     # for age comps, identifies the sequence of the normalizing, aggregation, and aging error process (matches what is done in ADMB sablefish currently)
                                          FishLen_comp_agg_type = c(1,1)       # for length comps, identifies the sequence of the normalizing, aggregation, and aging error process (matches what is done in ADMB sablefish currently)
)
...
input_list <- Setup_Mod_Weighting(input_list = input_list,
                                  sablefish_ADMB = 1,                          # ==1 indicates should use calcs as implemented in ADMB sablefish model for calc of indices (mainly LLS age comps is calculated end of year instead of mid-year mortality and JPN CPUE assumes only female selectivity--both issues rectified if set ==0)
                                  likelihoods = 0,                             # ==0 indicates should use same likelihoods as in ADMB sablefish model
                                  Wt_Catch = 50,                               # Weight (lambda) for catch data
                                  Wt_FishIdx = 0.448,                          # Weight (lambda) for fishery indices data
                                  Wt_SrvIdx = 0.448,                           # Weight (lambda) for survey indices data
                                  Wt_Rec = 1.5,                                # Weight (lambda) for stock-recruit penalty term
                                  Wt_F = 0.1,                                  # Weight (lambda) for fishing mortality penalty term
...
input_list <- Setup_Mod_Fishsel_and_Q(input_list = input_list,
                                      ...
                                      Use_fish_selex_prior = 0,                # Using selex priors
...
srv_selex_structure <- merge(fleet_blocks, sex_par) %>%
 dplyr::filter(!(fleet == 3 ))  %>%
  dplyr::filter(!(fleet == 1 & block == 2 & sex == 1 & par == 2)) %>%    # remove priors for any unestimated pars -- par1=a50, par2=delta; NEEDS TO MATCH PARAMETER MAPPING
 dplyr::filter(!(fleet == 1 & block == 2 & sex == 2 & par == 2))    # remove priors for any unestimated pars -- par1=a50, par2=delta; NEEDS TO MATCH PARAMETER MAPPING
...
input_list <- Setup_Mod_Srvsel_and_Q(input_list = input_list,
                                     ...
                                     srv_q_spec =                              # Whether to estimate all fixed effects for survey catchability
                                       c("est_all",                            # Estimate q for all survey fleets
                                         "est_all",
                                         "est_all"),
                                     Use_srv_selex_prior = 0,                 # Using selex priors
...
# CONT SETUP
 input_list$map$ln_fish_fixed_sel_pars <- factor(c(1:7, 2, 8:11,                # sharing logistic delta across sexes from early LLF fishery (first time block), all other pars estimated for LLF across all time blocks
                                                 ...
                                                 rep(c(14,15),3)))            # for trawl fishery males, share the delta par of the gamma function with the female estimate par (repeat 3 times to fill the 3 time blocks associated with the LLF)
...
input_list$map$ln_srv_fixed_sel_pars <-
  factor(c(1:3, 2,                                                             # domestic ll survey (2 time blocks), share delta female (par 2) across time blocks and to the coop jp ll survey delta (survey gear 3 below)
           4:6, 5,                                                             # domestic ll survey (2 time blocks), share delta male (par 5) across time blocks and to the coop jp ll survey delta (survey gear 3 below)
           rep(7,4),                                                           # domestic trawl survey (single time block) and only one estimated parameter (exponential selex fxn), so only one parameter estimated per sex across blocks (so repeat value 4 times for each sex)
           rep(8, 4),                                                          # domestic trawl survey (single time block) and only one estimated parameter (exponential selex fxn), so only one parameter estimated per sex across blocks (so repeat value 4 times for each sex)
           rep(c(NA,2), 2),                                                    # coop JPN LL survey (single time block) does not estimate parameters and shares deltas with longline survey by sex; a50 (first par) is fixed, so use NA to ensure not estimated in RTMB
           rep(c(NA, 5), 2)))                                                  # coop JPN LL survey (single time block) does not estimate parameters and shares deltas with longline survey by sex; a50 (first par) is fixed, so use NA to ensure not estimated in RTMB
...
#           rep(c(13:14),2)))                                                  # coop JPN LL survey (single time block, males)

# Selex starting values
#input_list$par$ln_srv_fixed_sel_pars[1,,,,] <- log(3)
#input_list$par$ln_fish_fixed_sel_pars[1,2,,,1] <- log(3)
#input_list$par$ln_fish_fixed_sel_pars[1,2,,,2] <- log(6)

# Coop JP Survey (flt3) (Logistic selex) Single time block uses fixed input pars for a50 so need to specify it here
input_list$par$ln_srv_fixed_sel_pars[1,,,1,3] <- c(0.980660760456, 0.9295241)  # NOTE: here inputting both a50 and delta, but only a50 being used (delta is taken from the estimated Dom LLS delta); also the array call here is pars(rows) X time blocks(Cols)--so using same par value for each time block
