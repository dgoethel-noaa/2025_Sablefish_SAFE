...
mod_name <- '25_14_TV_Selex'                                                    # Model name to use for saving files and labels
root_folder <- "Sept PT Model Runs"                                            # The root folder where runs will be stored (sub folders named with mod_name)

do_francis <- 0                                                                # whether or not to do francis reweighting for this run, ==0 NO, ==1 YES (NOT DOING FRANCIS!)
...
input_list <- Setup_Mod_Biologicals(input_list = input_list,
                                    ...
                                    M_prior = c(0.085, 0.1),                     # mean and sd for M prior
...
fleet_blocks <- data.frame(
  fleet = c(1, 2),                                                       # fleets corresponding to time blocks (3 fixed gear time blocks, no trawl gear blocks)
...
input_list <- Setup_Mod_Fishsel_and_Q(input_list = input_list,
                                      cont_tv_fish_sel =                       # whether continuous time-varying selectivity is used for either fishery fleet
                                        c("iid_Fleet_1", "none_Fleet_2"),
                                      fishsel_pe_pars_spec =
                                        c("fix", 'none'),                      # whether process error parameters associated w/ selex devs are estimated
                                      fish_sel_devs_spec =
                                        c("est_all", "none"),                  # whether selectivity deviations are estimated
                                      fish_sel_blocks =                        # fishery selectivity time blocks if not TV specified above for a given fleet
                                        c(
                                          "none_Fleet_1",                   # no blocks for fixed gear
...
# fixing selectivity process error parameters
input_list$par$fishsel_pe_pars[1,4,,1] <- log(0.3) # region, parameter, sex, fleet
...
# CONT SETUP
input_list$map$ln_fish_fixed_sel_pars <- factor(c(1:6, 5, 7)) # share trawl bmax
# Est All
...
input_list$map$ln_srv_fixed_sel_pars <-
  factor(c(1:8,                                                                # domestic ll survey (2 time blocks, 2 sexes),
           rep(NA,4),                                                          # domestic trawl survey (single time block) and only one estimated parameter (exponential selex fxn), so only one parameter estimated per sex across blocks (so repeat value 4 times for each sex)
           rep(NA, 4),                                                        # domestic trawl survey (single time block) and only one estimated parameter (exponential selex fxn), so only one parameter estimated per sex across blocks (so repeat value 4 times for each sex)
