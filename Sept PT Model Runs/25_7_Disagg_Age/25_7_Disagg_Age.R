...
mod_name <- '25_7_Disagg_Age'                                              # Model name to use for saving files and labels
...
SPoRC_one_reg <- readRDS(here(root_folder,mod_name,"SPoRC_one_reg_age_joint.rds"))
...
fish_selex_prior_tf <- fish_selex_prior %>%                                    # set tighter selex prior for TF
  ...
  dplyr::full_join(fish_selex_prior %>%  dplyr::filter(!(fleet == 2)))
